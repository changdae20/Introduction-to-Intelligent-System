...
int main(int argc, char** argv){
    ...
    while(running){
	    ...
	    */
        ...
        ros::spinOnce();
        control_rate.sleep();
        if(hypot(robot_pose.x - path_RRT[look_ahead_idx].x,robot_pose.y - path_RRT[look_ahead_idx].y)<(look_ahead_idx == path_RRT.size()-1 ? 0.2 : 0.5) && look_ahead_idx < path_RRT.size()){
            look_ahead_idx++;
            pid_ctrl.reset();
        }
